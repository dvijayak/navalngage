...
public:
	...
	MatrixF operator+ (const MatrixF<M, N>&) const;
	MatrixF operator- (const MatrixF<M, N>&) const;
...
template <size_t M, size_t N>
MatrixF<M, N>::MatrixF (const Vector2F& v)
	: m_nRows(M)
	, m_nCols(N)
{
	// ASSERT: A Vector2F must only be understood as a 2 x 1 or 1 x 2 matrix
	assert( (m_nRows == 2 && m_nCols == 1) || (m_nRows == 1 && m_nCols == 2) );
...
template <size_t M, size_t N>
MatrixF<M, N> MatrixF<M, N>::operator+ (const MatrixF<M, N>& other) const
{
...
MatrixF<M, O> operator* (const MatrixF<M, N>& A, const MatrixF<N, O>& B)
{
...
template <size_t M, size_t N>
MatrixF<M, N> MatrixF<M, N>::operator* (const float scalar) const
{
	MatrixF<M, N>::values_type values;
	for (size_t i = 0; i < M*N; ++i)
	{
